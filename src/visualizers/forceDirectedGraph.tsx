...
export const ForceDirectedGraph: React.FC<ForceDirectedGraphProps> = ({ codeMap, config, onNodeSelect }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      defs.select('#glow').append('feMerge').selectAll('feMergeNode').data([0, 1]).enter().append('feMergeNode').attr('in', (d: any) => (d ? 'SourceGraphic' : 'coloredBlur'))

      // Add circles with glow
      const circles = nodeGroups
        ...
        .attr('fill-opacity', (d: any) => {
          ...
          if (config.colorMode === 'blast' && blastRadiusRef.current) {
            ...
            return affected.has(d.id) ? 1.0 : 0.2
          }
          return 1.0
        })
        .attr('stroke', '#fff')
